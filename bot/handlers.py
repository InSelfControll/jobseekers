...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return ConversationHandler.END
async def request_contact_details(telegram_id: str):
    """Request contact details from job seeker"""
    ...
    await context.bot.send_message(
        chat_id=telegram_id,
        text="An employer would like to contact you via WhatsApp. Please share your contact information.",
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if contact:
        with app.app_context():
            ...
            if job_seeker:
                job_seeker.whatsapp_number = contact.phone_number
                job_seeker.telegram_username = update.effective_user.username
                db.session.commit()
                await update.message.reply_text(
                    "Thank you! The employer will contact you via WhatsApp shortly.",
                    reply_markup=ReplyKeyboardRemove()
                )

async def send_test_to_candidate(telegram_id: str, test_content: str, duration: int, application_id: int):
    """Send test to candidate"""
    await context.bot.send_message(
        chat_id=telegram_id,
        text=f"You have received a test for your job application!\n\n"
             f"Duration: {duration} minutes\n\n"
             f"Test content:\n{test_content}\n\n"
             f"Please complete the test within the given time limit."
    )
